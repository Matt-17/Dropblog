...
class AdminController implements ControllerInterface
{
    ...
    private function authenticate(): array|bool
    {
    ...
    public function handleUpdate(): array
    {
        ...
        // Check if migrations table exists, if not, we're on a fresh database
        $migrationsTableExists = false;
        try {
            $stmt = $this->pdo->query("SELECT 1 FROM migrations LIMIT 1");
            $migrationsTableExists = true;
        } catch (\PDOException $e) {
            // Table doesn't exist, which is fine for fresh installations
            $migrationsTableExists = false;
        }
        ...
        foreach ($migrations as $migration) {
            ...
            // For fresh databases, skip the check on the first migration (should be 001_create_migrations_table.sql)
            $shouldSkipCheck = !$migrationsTableExists && str_starts_with($filename, '001_');
            ...
            if (!$shouldSkipCheck) {
                ...
                try {
                    ...
                    // If we can't check, assume we need to run it (for edge cases)
                    // This should only happen if the migrations table exists but is corrupted
                }
            }
            ...
            try {
                ...
                // After running the first migration, the migrations table should exist
                if (!$migrationsTableExists && str_starts_with($filename, '001_')) {
                    $migrationsTableExists = true;
                }
                
                // Record the migration as applied (skip if migrations table still doesn't exist)
